...
fn parse_segments<R: Read + Seek>(
    reader: &mut R,
    descriptor: &mut Descriptor,
    header: &ElfHeader,
) -> ParseElfResult<Vec<Segment>> {
...
fn associate_string_table(
    section_map: &mut HashMap<String, usize>,
    sections: &[Section],
    header: &ElfHeader,
) {
    if let Some(idx) = header.section_name_table_index() {
        if let Some(section) = sections.get(idx) {
            if let SectionData::Strings(table) = section.data() {
                for (i, section) in sections.iter().enumerate() {
                    let name_idx = section.header().name_index();
                    if let Some(name) = table.get(name_idx) {
                        section_map.insert(name.clone(), i);
                    }
                }
            }
        }
    ...
    fn segment_iters() {
        ...
        for (i, h) in elf.segments().enumerate() {
            match i {
                ...
                1 => assert_eq!(
                    h.header().program_header_type(),
                    ProgramHeaderType::Loadable
                ),
                2 => assert_eq!(
                    h.header().program_header_type(),
                    ProgramHeaderType::Loadable
